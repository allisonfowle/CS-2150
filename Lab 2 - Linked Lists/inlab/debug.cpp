...
int main() {
    ...
    for ( nCount = 0; nCount < MAX; nCount++ ) {
        ...
        cin >> nValues[nCount]; //1 to nCount to add values to array list called nValues
    }

    // Now we echo the input back to the user
    for ( nCount = 0; nCount < MAX; nCount++) { //change array list start value to 0
...
double GetAverage(int nIn[], int nMax) {
    ...
    double temp = 0.0;
    for ( int i = 0; i < nMax; i++) // > to < while i is less than nMax continue the loop
        temp += nIn[i];
    temp /= nMax;
